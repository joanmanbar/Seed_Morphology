...
Multiple USDA PI accessions with different genome sizes were used to explore a potential relationship between seed size and ploidy level in Festuca species. Seeds from each of the three different accessions within ploidy level (2X, 4X, 6X and 8X) were randomly selected to measure the area, length and width of the seeds using image analysis. The varieties "Beacon" (6X) and "Quatro" (4X) were used as controls, for a total of 14 genotypes. The TIF images (Tagged Image File Format) were obtained by scanning the seeds with an Epson Perfection V6 flatbed scanner at 1200 dpi (dots per inch) with size 2097x1624 pixels, which corresponds to a height=44mm and width=34mm, respectively. The images were processed with a custom Matlab script (**https://github.com/joanmanbar/Seed_Morphology**) that transformed the original images to threshold and measure the seed length and width respectively as the length (in pixels) of the major and minor axes of a fitted ellipse, whereas the area is calculated as the number of pixels in the seed. The script returned a dataset containing 10 observations for each of the 14 accessions that was used to regress the seed size on the ploidy level using R (R Core Team, 2014), and plot figures using the ggplot2 package (Wickham, 2009).


\color{red}
Yinjie, I don't think "Beacon" and "Quatro" should be consider controls in this case. Let's talk about it.
\color{black}
...
# Read data
mydata1 <- read.csv(paste0(mydir, '/Image_Analysis/data.csv'))
